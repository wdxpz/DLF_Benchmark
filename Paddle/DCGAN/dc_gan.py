...
def train(args):
    ...
    image_filename = 'data/data65/train-images-idx3-ubyte.gz'
    label_filename = 'data/data65/train-labels-idx1-ubyte.gz'
    ...
    RESULT_FILE = 'result/results_dcgan.txt'
    ...
    index = 0
    while os.path.exists(os.path.join('result/mnist', 'minst_dcgan_tf_epoch_{}_{}.png'.format(args.epoch, index))):
        index += 1
    imgname = os.path.join('result/mnist', 'minst_dcgan_tf_epoch_{}_{}.png'.format(args.epoch, index))
    generated_images = exe.run(g_program_test,
                                feed={'noise': const_n},
                                fetch_list=[g_img])[0]
    total_images = np.concatenate([real_image, generated_images])
