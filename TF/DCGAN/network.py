...
def make_generator_model():
    ...
    assert model.output_shape == (None, 8, 8, conf['ngf']*8)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    model.add(layers.Conv2DTranspose(conf['ngf']*4, (5, 5), strides=(2, 2), padding='same', use_bias=False))
    assert model.output_shape == (None, 16, 16, conf['ngf']*4)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    model.add(layers.Conv2DTranspose(conf['ngf']*2, (5, 5), strides=(2, 2), padding='same', use_bias=False))
    assert model.output_shape == (None, 32, 32, conf['ngf']*2)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
...
def make_discriminator_model():
    ...
    model.add(layers.Conv2D(conf['ndf']*16, (5, 5), strides=(2, 2), padding='same'))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.2))
